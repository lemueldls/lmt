...
type ParserInput<'src> = chumsky::input::MappedInput<
    Token<'src>,
    SimpleSpan,
    &'src [(Token<'src>, SimpleSpan)],
    Box<
        dyn for<'x> Fn(
            &'x (Token<'_>, chumsky::span::SimpleSpan),
        ) -> (&'x Token<'x>, &'x SimpleSpan),
    >,
    // fn(&'src (Token<'src>, SimpleSpan)) -> (&'src Token<'src>, &'src SimpleSpan),
>;

trait TokenParser<'src, I> =
    Parser<'src, ParserInput<'src>, Spanned<I>, extra::Err<Error<'src, Token<'src>>>> + Clone;

fn ident<'src>() -> impl TokenParser<'src, String> {
    select! { Token::Ident(ident) => ident }
        .map_with(|ident, e| Spanned::new(ident.to_owned(), e.span()))
        .labelled("identifier")
}

fn type_annotation<'src>() -> impl TokenParser<'src, TypeAnnotation> {
    ident()
        .map_with(|ident, e| Spanned::new(TypeAnnotation::Expr(Expr::Ident(ident)), e.span()))
        .labelled("type annotation")
}

fn typed_ident<'src>() -> impl TokenParser<'src, TypedIdent> {
    ident()
        .then(
            just(Token::Ctrl(':'))
...
fn path<'src>() -> impl TokenParser<'src, PathBuf> {
...
fn stmt_parser<'src>() -> impl TokenParser<'src, Stmt> {
    recursive(|stmt| {
...
fn expr_parser<'src>(stmt: impl TokenParser<'src, Stmt> + 'src) -> impl TokenParser<'src, Expr> {
    let ident = ident();
...
fn block<'src>(
    stmt: impl TokenParser<'src, Stmt>,
    expr: impl TokenParser<'src, Expr>,
) -> impl Parser<'src, ParserInput<'src>, Block, extra::Err<Rich<'src, Token<'src>, SimpleSpan>>> + Clone
{
...
fn function_parser<'src>(
    stmt: impl TokenParser<'src, Stmt>,
    expr: impl TokenParser<'src, Expr>,
) -> impl Parser<'src, ParserInput<'src>, Function, extra::Err<Rich<'src, Token<'src>, SimpleSpan>>>
+ Clone {
    let ident = ident();
    let typed_ident = typed_ident();
    ...
    let function = just(Token::Fn)
        ...
        .then(block(stmt, expr).map_with(|block, e| Spanned::new(block, e.span())))
...
pub fn module_parser<'src>() -> impl Parser<
    'src,
    ParserInput<'src>,
    Vec<Spanned<Stmt>>,
