...
fn main() -> Result<(), String> {
    ...
    let mut jit = JITObject::new("lmt.o");
    // jit.create_data("hello_string", "hello world!\0".as_bytes().to_vec())?;
    jit.compile(LMT_CODE).unwrap();
...
fn run_macro(jit: &mut jit::JIT) -> Result<isize, String> {
    unsafe { run_code(jit, MACRO_CODE, ()) }
}
